...
make_new_df <- function(i, media_df, col_df, ukey_df, return_summary = TRUE, return_df = FALSE ){
  ...
  if(NROW(new_df) > 0) {
    ...
    if(return_summary){
      i_df$summary <- list(new_df |> mutate(ad_date = as.Date(ad_date)) |> skimr::skim())
    }
...
saveRDS(object = out_df, file = "~/Documents/r_wd/adtabler/scripts/temp/out_tv_national.rds")

# source("~/Documents/r_wd/adtabler/scripts/temp/tv_national_make_new_df.R")

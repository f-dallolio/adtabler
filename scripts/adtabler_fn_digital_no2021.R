...
file_df <- list.files(adintel_dir, full.names = T, recursive = T) |>
  ...
  mutate(file_type = str_split_i(input_file, "/", -2) |> rename_adintel(),
         ...
         date_max1 = str_c(file_year ,"03", "31", sep = "-") |> as.Date(),
         date_min2 = date_max1 + 1,
         date_max2 = str_c(file_year ,"06", "30", sep = "-") |> as.Date(),
         date_min3 = date_max2 + 1,
         date_max3 = str_c(file_year ,"09", "30", sep = "-") |> as.Date(),
         date_min4 = date_max3 + 1,
         date_max4 = str_c(file_year + 1 ,"01", "06", sep = "-") |> as.Date()) |>
...
media_df <- media_type_table |>
  ...
  select(file_type, file_type2, media_type_id, file_year, input_file, new_dir, new_file, date_min, date_max) |>
  distinct() |>
  filter( file_type2 |> str_detect("digital"))
...
# source("~/Documents/r_wd/adtabler/scripts/adtabler_fn_digital_no2021.R")

...
get_adintel_tables <- function(adintel_dir){
  ...
  year_files <- adintel_all_files %>%
    ...
    mutate(year = str_split_i(adintel_files, "/", -3) %>% chr_to_num(),
  ...
  static_refereces_files <- adintel_all_files %>%
    filter(str_detect(adintel_files,
                      get_static_file(dir = adintel_dir,
                                      year = latest_year,
                                      dir_out = TRUE))) %>%
    transmute(year = -1,
  ...
  adintel_data_table_out <- adintel_data_table %>%
    mutate(file_type = str_sep_upper(adintel_file_type, "_") %>%
           ...
           file_name = str_sep_upper(adintel_file,"_") %>%
             ...
             str_replace_all("__", "_"),
           .after = year) %>%
    dplyr::mutate(is_dynamic = adintel_dynamic_flag,
           file_year = year,
  ...
  adintel_data_table_out %>% mutate(size_kb = file.size(full_path)/1024,
                                    KB = paste(round(size_kb,3), "KB"),
                                    size_mb = size_kb/1024,
                                    MB = paste(round(size_mb,3), "MB"),
                                    size_gb = size_mb/1024,
                                    GB = paste(round(size_gb,3), "GB")) %>%
    mutate(size = if_else(size_mb > 100, GB, MB), size_mb = round(size_mb),
           .before = full_path) %>%

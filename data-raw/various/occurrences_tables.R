...
occurrences_tables <- docxtractr::read_docx(occurrences_docx) |>
  ...
  mutate(
    adintel_file_type = "Occurrences",
    adintel_file_name = nms %>% str_split_i("-", -1) %>% str_squish(),
    .keep = "unused"
  ) %>%
  mutate(
    file_type = adintel_file_type %>% str_sep_upper("_") %>% str_squish(),
...
occurrences_tables <- occurrences_tables %>% rename_with(.cols = contains("nielsen"), .fn = ~ .x %>%
  str_remove_all("occurrences_") %>%
  str_remove_all("nielsen_") %>%
  paste("manual", sep = "_"))

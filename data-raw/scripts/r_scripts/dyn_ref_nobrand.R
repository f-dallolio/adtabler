...
ref_nobrand_df <- dyn_ref_df |>
  filter(file_type2 != "brand") |>
  filter(file_type2 |> str_detect("digital", negate = TRUE))
...
bad_out <- tibble(
  input_file = ref_nobrand_df$input_file,
...
# source("~/Documents/r_wd/adtabler/scripts/dyn_ref_nobrand.R")

...
fn_col_info <- function(x,.name){
  df <- data.table::fread(file = x,
                          nrows = 100)
  col_names_man <- names(df)
  n_cols <-NCOL(df)
  col_pos <- seq_along(col_names_man)
  col_names_std <- rename_adintel(col_names_man)
  tibble(n_cols,
         col_pos,
         col_names_man,
         col_names_std)
...
adintel_files <- bind_rows(adintel_files_dyn, 
...
The data frame `row_numbers` above is a pre-calculated table that contains the number of rows for each file. The number of rows was efficiently computed with the `read_nrows` function.

```{r}
#| eval: false
save(adintel_files, file = "~/Documents/r_wd/adtabler/data/adadintel_files.rda")
```

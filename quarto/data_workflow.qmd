...
```{r}
n_col_separate <- str_count(
  string = adintel_files_2020$full_file_name,
  pattern = "/"
) |> max()
names_col_separate <- paste0("V", seq(1, n_col_separate))
...
all_dyn_files <- adintel_files_2020 |> 
  mutate(
    ...
    file_name_std = rename_adintel(
      str_split_i(file_name, "\\.", 1)
    ) |> paste(str_split_i(file_name, "\\.", 2), sep = ".")
  )
```

...
references_columns <- references_columns |> 
  rename(datatype_man = col_class_man,
         # datatype_std = col_class_lower,
         sql_precision = col_p,
         sql_scale = col_n)
...
occurrences_columns <- occurrences_columns |> 
  rename(datatype_man = col_class_man,
         ...
         sql_precision = col_p,
         sql_scale = col_n,
         file_name_man = file_name) |> 
  mutate(file_name_std = file_name_man |> 
           str_remove_all(".tsv") |> 
           rename_adintel(),
         .before = 1) |> 
  mutate(col_name_std = rename_adintel(col_name_man),
         .after = col_pos) |> 
  relocate(file_name_man, col_name_man, datatype_man, .after = description)

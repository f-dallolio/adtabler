...
adintel_files <- bind_rows(adintel_files_dyn, adintel_files_static) |>
  ...
  mutate(
    ...
    col_names_std = map(col_names_man, rename_adintel)
  ) |>
  mutate(
    n_cols = map_int(col_names_man, length),
    col_pos = map(col_names_man, seq_along),
    .after = n_rows
  ) |> 
  relocate(full_file_name, .after = last_col())

write_csv(adintel_files, "~/Documents/r_wd/adtabler/data-raw/various_data/adinte_files_quarto.csv")
```

```{r}
adintel_files |> 
  mutate(across(year : file_name_std, as_factor)) |> 
